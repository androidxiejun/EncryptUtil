...
public class SM2Utils extends GMBaseUtils {
    ...
    /**
     * 通过16进制的公钥字符串生成ECPublicKeyParameters格式的公钥对象
     * @param pubKeyHexStr  公钥的16进制字符串
     * @return
     */
    public static ECPublicKeyParameters publicKeyFromHexStr(String pubKeyHexStr) {
        ECCurve localECCurve = GMNamedCurves.getByName("sm2p256v1").getCurve();
        X9ECPoint localX9ECPoint = new X9ECPoint(localECCurve, ByteUtils.fromHexString(pubKeyHexStr));
        ...
        return new ECPublicKeyParameters(localX9ECPoint.getPoint(), domainParams);
    }
